...
public class Date {
	...
	private boolean esElDia(int dia){
		return ((dia>0) && (dia<=this.diasDelMes(this._mes)));
	}
	/*Metodo que retorna el dia del mes*/
	public int diasDelMes(){
		return this.diasDelMes(this._mes);
	}
	...
	public boolean isSameYear (Date other){
		return this._anio==other.getAnio();
	}
	...
	public String toString(){
        ...
        try{
        	do{
        		...
        		d = (int) (Math.random()*this.diasDelMes(m) ) + 1;
        		y = this._anio;
        		tries++;
	...
	public String getMonthsLeft(){
		...
		try{
			for (int i = this._mes + 1; i <= 12; i++){
				aux.setMonth(i);
				monthsLeft.append(aux.getNombreMes() + " ");
			}
		} catch (DateException e){
	...
	public String getMonthsSameDays(){
		...
		try{
			for ( int i = 1; i <= 12; i++) {
				aux.setMonth(i);
				if ( aux.diasDelMes() == this.diasDelMes() ) {
					months.append(aux.getNombreMes() + " ");
				}
			}
		} catch (DateException e){
